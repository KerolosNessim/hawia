...
type FaqDetailsListProps = {
  ...
  pageName?: string;
  /** When false, only visible HTML is emitted (pair with JSON-LD on service pages). */
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      {items.map((item, index) => {
        ...
        const questionProps = useMicrodata
          ? { itemProp: "name" as const }
          : {};
        ...
        return (
          ...
          >
            ...
            >
              <span className="flex-1">{questionLabel}</span>
              <span
                aria-hidden
